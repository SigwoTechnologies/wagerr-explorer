...
const peerlessResult = {
  ...
  sResult: buildProperty('Decimal', 4),
  tResult: buildProperty('Decimal', 4),
};
...
async function handleMappingData(hex, fieldData) {
  let decryptedSection = bufferDecrypt(hex);
  let mappingNameData;
...
async function decode(hexValue, decodeType = 'hex') {
  ...
  for (let x = 0; x < fieldsToFill.length; x +=1) {
    ...
    if (dataType === 'Decimal' && fieldData.mappedData) {
      ...
      mappingNameData = await handleMappingData(thisSection, fieldData);
