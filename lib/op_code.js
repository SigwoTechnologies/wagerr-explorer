  ...
  try {
    ...
    if (!query) {
      ...
      try {
        ...
        throw new Error('Could not save mapping data');
      }
    } else {
      mappingNameData = {
    ...
    log(`Could not save mapping data => mappingIndex: ${fieldData.mappedData}, mappingId: ${decryptedSection})}`);
    throw new Error(e);
  }
...
async function decode(hexValue, decodeType = 'hex', showLogs = false) {
  ...
  for (let x = 0; x < fieldsToFill.length; x +=1) {
    ...
    if (showLogs) {
      log(`=====> Field '${f}'`);
      log(`thisSection => ${thisSection}`);
      log(`decryptedValue =>  ${decryptedValue}`);
      if (mappingId) {
        log(`mappingId =>  ${mappingId}`);
      }
      log('--------------------------------------');
    }
