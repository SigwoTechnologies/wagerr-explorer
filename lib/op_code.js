...
async function handleSingleMappingRequest(id, fieldData) {
  let mappingNameData;
  ...
  try {
    ...
    if (!query) {
      ...
      try {
        ...
        // console.log(mappingNameData);
        console.log('Could not save mapping data');
        console.log(`Mapping index is ${decryptedSection} of ${fieldData.mappedData} - mapping-name ${mappingNameData['mapping-index']}`);
        // console.log(e);
      }
    } else {
      mappingNameData = {
    ...
    console.log(`Could not save mapping data => mappingIndex: ${fieldData.mappedData}, mappingId: ${decryptedSection})}`);
    console.log(e);
  }
...
async function handleMappingData(hex, fieldData) {
  let decryptedSection = decryptFromLittleEndian(hex);
  let mappingNameData;
  try {
    mappingNameData = await handleSingleMappingRequest(decryptedSection, fieldData);
  } catch (e) {
    console.log('First decryption method for mapping data failed');
...
async function decode(hexValue, decodeType = 'hex') {
  ...
  for (let x = 0; x < fieldsToFill.length; x +=1) {
    ...
    if ((dataType === 'Decimal' || dataType === 'LittleEndian') && fieldData.mappedData) {
      let mappingNameData = {};
      console.log('This is the mapping data');
      console.log(thisSection);
      mappingNameData = await handleMappingData(thisSection, fieldData, dataType);
    ...
    console.log(thisSection, decryptedValue, f);
